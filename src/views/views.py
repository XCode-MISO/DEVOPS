...
def post_add_email_to_blacklist(db, request):
    try:
        ...
        new_blacklisted = Blacklisted(
            ...
            time=datetime.now()  # Usando el objeto datetime directamente
        )
...
def get_blacklisted_entries(db, request, email):
    try:
        ...
        blackmailed = db.session.query(Blacklisted).filter_by(email=email).first()
        if blackmailed:
            return {
                "blacklisted": True,
                "blocked_reason": str(blackmailed.blocked_reason)
            }, 200
